...
export default function Hero() {
  return (
    <section className="relative bg-navy min-h-screen flex flex-col">
      ...
      <div className="relative flex-1 flex items-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 text-center w-full">
      ...
      <div className="relative border-t border-gold/30 bg-navy/80 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-2 lg:grid-cols-4 gap-6 text-center">
          {[
            { number: "25+", label: "Years Experience" },
            { number: "1,000+", label: "Cases Won" },
            { number: "Free", label: "Initial Consultations" },
            { number: "$50M+", label: "Recovered" },
          ].map((stat) => (
            <div key={stat.label}>
              <div className="font-serif text-3xl sm:text-4xl text-gold tracking-tight">{stat.number}</div>
              <div className="text-white/60 text-xs uppercase tracking-[0.2em] mt-1">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
